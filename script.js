(() => {
  ...
  const BOOTSTRAP_CLIENT_PACKET = Object.freeze({ JOIN: 1 });
  const BOOTSTRAP_SERVER_PACKET = Object.freeze({ INIT: 1, NOTICE: 6 });
  let CLIENT_PACKET = BOOTSTRAP_CLIENT_PACKET;
  let SERVER_PACKET = BOOTSTRAP_SERVER_PACKET;
  let ITEM_ID = Object.freeze({});
  let RESOURCE = Object.freeze({});
  let BUILDING_RELATION = Object.freeze({});
  const {
    clamp,
    normalizeAngle,
    lerpAngle,
  ...
  const ACTIONS = ["sword", "hammer", "apple", "spike", "trap", "wall", "gatherer", "autohit"];
  const ACTION_LABELS = {
    sword: "Equip katana",
    hammer: "Equip hammer",
    apple: "Use apple",
    ...
    autohit: "Toggle autohit"
  };
  let ACTION_ITEM_IDS = Object.freeze({});
  let ITEM_ACTIONS = Object.freeze({});
  ...
  let BUILDING_INDICATOR_COLORS = Object.freeze({});
  const SPRITE_RENDER_SCALE = VISUALS.SPRITE_SCALE;
  const WORLD_RENDER_RADIUS = 1000;
  const REFERENCE_VIEW_WIDTH = 1920;
  const REFERENCE_VIEW_HEIGHT = 1080;
  const LOCAL_INTERPOLATION_BASE_TICKS = 2;
  const REMOTE_INTERPOLATION_BASE_TICKS = 3;
  const LOCAL_INTERPOLATION_MIN_MS = 90;
  const REMOTE_INTERPOLATION_MIN_MS = 125;
  const LOCAL_INTERPOLATION_MAX_MS = 180;
  const REMOTE_INTERPOLATION_MAX_MS = 250;
  const SNAPSHOT_TIMING_SMOOTHING = 0.12;
  const SNAPSHOT_JITTER_SMOOTHING = 0.16;
  const REMOTE_ANGLE_LERP_RATE = 0.009;
  const HEALTH_TRAIL_LERP_RATE = 0.016;
  const INVENTORY_VISUALS = VISUALS.INVENTORY;
  let INVENTORY_ORDER = Object.freeze([]);
  const INVENTORY_SLOTS = INVENTORY_VISUALS.slots;
  let RESOURCE_TYPES = Object.freeze(Object.keys(VISUALS.RESOURCES));
  const BUILD_TYPES = Object.freeze(Object.keys(VISUALS.BUILDS));
  const PROTOCOL_KEYS = Object.freeze({
    ITEM: Object.freeze(["SWORD", "HAMMER", "APPLE", "SPIKE", "TRAP", "WALL", "GATHERER"]),
    RESOURCE: Object.freeze(["TREE", "STONE", "GOLD", "BUSH"]),
    BUILDING_RELATION: Object.freeze(["ENEMY", "CLANMATE", "OWNER"]),
    CLIENT: Object.freeze([
      "JOIN", "MOVE", "AIM", "PLACE_BUILDING", "ATTACK", "EQUIP_WEAPON",
      "USE_APPLE", "CREATE_CLAN", "REQUEST_CLAN_JOIN", "ANSWER_CLAN_REQUEST",
      "KICK_CLAN_MEMBER", "LEAVE_CLAN", "PING", "CHAT", "BUY_HELMET",
      "TOGGLE_HELMET"
    ]),
    SERVER: Object.freeze(["INIT", "SNAPSHOT", "LEADERBOARD", "CLAN_DATA", "PONG", "NOTICE"])
  });
  ...
  function copyProtocolSection(protocol, sectionName, valueType) {
    const source = protocol?.[sectionName];
    const keys = PROTOCOL_KEYS[sectionName];
    if (!source || typeof source !== "object" || !keys) return null;

    const copy = {};
    for (const key of keys) {
      const value = source[key];
      const valid = valueType === "number"
        ? Number.isInteger(value) && value >= 0
        : typeof value === "string" && value.length > 0;
      if (!valid) return null;
      copy[key] = value;
    }
    if (new Set(Object.values(copy)).size !== keys.length) return null;
    return Object.freeze(copy);
  }

  function installServerProtocol(protocol) {
    if (!protocol || protocol.VERSION !== 1) return false;

    const nextItem = copyProtocolSection(protocol, "ITEM", "number");
    const nextResource = copyProtocolSection(protocol, "RESOURCE", "string");
    const nextRelation = copyProtocolSection(protocol, "BUILDING_RELATION", "number");
    const nextClient = copyProtocolSection(protocol, "CLIENT", "number");
    const nextServer = copyProtocolSection(protocol, "SERVER", "number");
    if (!nextItem || !nextResource || !nextRelation || !nextClient || !nextServer) return false;

    if (
      nextClient.JOIN !== BOOTSTRAP_CLIENT_PACKET.JOIN ||
      nextServer.INIT !== BOOTSTRAP_SERVER_PACKET.INIT ||
      nextServer.NOTICE !== BOOTSTRAP_SERVER_PACKET.NOTICE
    ) return false;

    const nextActionItems = Object.freeze({
      sword: nextItem.SWORD,
      hammer: nextItem.HAMMER,
      apple: nextItem.APPLE,
      spike: nextItem.SPIKE,
      trap: nextItem.TRAP,
      wall: nextItem.WALL,
      gatherer: nextItem.GATHERER
    });
    const visualActions = INVENTORY_VISUALS.order;
    if (
      !Array.isArray(visualActions) ||
      visualActions.some((action) => !Number.isInteger(nextActionItems[action]) || !INVENTORY_SLOTS[action])
    ) return false;

    const nextResourceTypes = Object.values(nextResource);
    if (nextResourceTypes.some((type) => !VISUALS.RESOURCES[type])) return false;

    CLIENT_PACKET = nextClient;
    SERVER_PACKET = nextServer;
    ITEM_ID = nextItem;
    RESOURCE = nextResource;
    BUILDING_RELATION = nextRelation;
    ACTION_ITEM_IDS = nextActionItems;
    ITEM_ACTIONS = Object.freeze(
      Object.fromEntries(Object.entries(ACTION_ITEM_IDS).map(([action, id]) => [id, action]))
    );
    BUILDING_INDICATOR_COLORS = Object.freeze({
      [BUILDING_RELATION.ENEMY]: "#ff4d4d",
      [BUILDING_RELATION.CLANMATE]: "#ffd84a",
      [BUILDING_RELATION.OWNER]: "#8fe63f"
    });
    INVENTORY_ORDER = Object.freeze(visualActions.map((action) => ACTION_ITEM_IDS[action]));
    RESOURCE_TYPES = Object.freeze(nextResourceTypes);
    return true;
  }
  ...
  const storedHotkeyVersion = Number(localStorage.getItem("swordfight.hotkeyVersion"));
  const storedHotkeysRaw = storedHotkeyVersion >= 2
    ? loadStoredJson("swordfight.hotkeys", {})
    : {};
  const storedHotkeys = Object.fromEntries(
    ACTIONS
      .filter((action) => (
        typeof storedHotkeysRaw[action] === "string" &&
        normalizeKey(storedHotkeysRaw[action]) !== "space"
      ))
      .map((action) => [action, normalizeKey(storedHotkeysRaw[action])])
  );
  ...
  function resizeCanvas() {
    ...
    const fitScale = Math.min(
      1,
      cssWidth / REFERENCE_VIEW_WIDTH,
      cssHeight / REFERENCE_VIEW_HEIGHT
    );
    game.viewportScale = Math.max(0.1, fitScale);
    canvas.width = Math.max(1, Math.round(cssWidth / game.viewportScale));
    canvas.height = Math.max(1, Math.round(cssHeight / game.viewportScale));
  ...
  function connectToServer() {
    ...
    socket.addEventListener("open", () => {
      ...
      send([BOOTSTRAP_CLIENT_PACKET.JOIN, nickname]);
    });
  ...
  function resetGameState() {
    ...
    game.selectedItemId = ITEM_ID.SWORD ?? null;
    game.localAimAngle = 0;
    game.camera.x = 0;
    game.camera.y = 0;
    game.mouseDown = false;
    game.spaceDown = false;
    game.autohitActive = false;
    ...
    game.snapshotTiming.lastArrivalAt = null;
    game.snapshotTiming.gapMs = game.tickMs;
    game.snapshotTiming.jitterMs = 0;
  ...
  function inventoryMetadataById(itemId) {
    const action = ITEM_ACTIONS[itemId];
    return action ? VISUALS.INVENTORY?.slots?.[action] || null : null;
  }

  function hydrateClientConfig(serverConfig) {
    const rawConfig = serverConfig && typeof serverConfig === "object" ? serverConfig : {};
    const { PROTOCOL: ignoredProtocol, ...config } = rawConfig;
  ...
  function enterGame(init) {
    if (!installServerProtocol(init?.config?.PROTOCOL)) {
      game.mode = "lobby";
      setConnectingUi(false);
      setLobbyError("This server uses an incompatible game protocol.");
      if (game.socket?.readyState === WebSocket.OPEN) game.socket.close(4004, "incompatible protocol");
      return false;
    }
  ...
  function handleServerMessage(raw) {
    ...
    if (game.mode === "connecting") {
      if (packet[0] === BOOTSTRAP_SERVER_PACKET.INIT) {
        if (packet[1] && Number.isInteger(packet[1].selfId) && packet[1].config) enterGame(packet[1]);
      } else if (packet[0] === BOOTSTRAP_SERVER_PACKET.NOTICE && typeof packet[1] === "string") {
        setLobbyError(packet[1]);
      }
      return;
    }
    ...
    switch (packet[0]) {
      ...
      case SERVER_PACKET.NOTICE:
        if (typeof packet[1] === "string") showToast(packet[1]);
        break;
      default:
        break;
    }
  ...
  function updateSnapshotTiming(now, tickDelta) {
    const timing = game.snapshotTiming;
    const expectedGap = Math.max(game.tickMs, tickDelta * game.tickMs);
    if (Number.isFinite(timing.lastArrivalAt)) {
      const observedGap = Math.max(0, now - timing.lastArrivalAt);
      const jitterSample = Math.abs(observedGap - expectedGap);
      timing.gapMs += (observedGap - timing.gapMs) * SNAPSHOT_TIMING_SMOOTHING;
      timing.jitterMs += (jitterSample - timing.jitterMs) * SNAPSHOT_JITTER_SMOOTHING;
    } else {
      timing.gapMs = expectedGap;
      timing.jitterMs = 0;
    }
    timing.lastArrivalAt = now;
  }

  function interpolationDurationMs(isLocalPlayer, tickDelta = 1) {
    const expectedSpan = Math.max(game.tickMs, tickDelta * game.tickMs);
    const baseTicks = isLocalPlayer
      ? LOCAL_INTERPOLATION_BASE_TICKS
      : REMOTE_INTERPOLATION_BASE_TICKS;
    const minimum = isLocalPlayer
      ? LOCAL_INTERPOLATION_MIN_MS
      : REMOTE_INTERPOLATION_MIN_MS;
    const maximum = isLocalPlayer
      ? LOCAL_INTERPOLATION_MAX_MS
      : REMOTE_INTERPOLATION_MAX_MS;
    const jitterPadding = game.snapshotTiming.jitterMs * (isLocalPlayer ? 1.15 : 1.65);
    const pingAboveLan = clamp((Number.isFinite(game.ping) ? game.ping : 50) - 40, 0, 110);
    const latencyPadding = pingAboveLan * (isLocalPlayer ? 0.08 : 0.15);
    return clamp(
      Math.max(expectedSpan, game.tickMs * baseTicks) + jitterPadding + latencyPadding,
      minimum,
      maximum
    );
  }
  ...
  function createRenderPlayer(row, now = performance.now(), serverTick = game.serverTick) {
    ...
    return {
      ...
      snapshotDurationMs: interpolationDurationMs(row[0] === game.selfId),
  ...
  function applyGameData(data) {
    ...
    const previousServerTick = game.serverTick;
    const snapshotTickDelta = previousServerTick >= 0
      ? Math.max(1, data.serverTick - previousServerTick)
      : 1;
    sampleTickRate(data.serverTick, snapshotNow);
    updateSnapshotTiming(snapshotNow, snapshotTickDelta);
    ...
    for (const row of data.players) {
      ...
      if (!player) {
        ...
        player.snapshotDurationMs = interpolationDurationMs(
  ...
  function inventorySlotForItem(itemId) {
    const action = ITEM_ACTIONS[itemId];
    return action ? INVENTORY_SLOTS[action] || null : null;
  }
  ...
  function selectItem(itemId) {
    ...
    if (game.selectedItemId === itemId) {
      ...
      } else if (["tool", "weapon"].includes(definition.kind)) {
        syncAttackIntent();
      }
      return;
    }
  ...
  function captureHotkey(event) {
    ...
    if (key === "space") {
      showToast("Space is reserved as the alternate use / attack key.");
      return true;
    }
  ...
  function selectedItemCanAttack() {
    const definition = getDefinition(game.selectedItemId);
    return Boolean(definition && (definition.kind === "tool" || definition.kind === "weapon"));
  }

  function attackShouldBeActive() {
    return game.mode === "game" &&
      game.players.has(game.selfId) &&
      selectedItemCanAttack() &&
      (game.mouseDown || game.spaceDown || game.autohitActive);
  }

  function stopAttackIntent(force = false) {
    if (!game.attackIntentActive) return false;
    if (!force && attackShouldBeActive()) return false;
    game.attackIntentActive = false;
  ...
  function syncAttackIntent(now = performance.now()) {
    if (!attackShouldBeActive()) {
      stopAttackIntent(true);
      return false;
    }
    return tryStartHitting(now);
  }
  ...
  function placeSelectedBuilding(definition) {
  ...
  function pressPrimaryAction(source, now = performance.now()) {
    if (game.mode !== "game" || game.openModal || !dom.chatComposer.hidden) return;
    const definition = getDefinition(game.selectedItemId);
    if (!definition) return;

    if (definition.kind === "tool" || definition.kind === "weapon") {
      if (source === "mouse") game.mouseDown = true;
      if (source === "space") game.spaceDown = true;
      syncAttackIntent(now);
    } else if (definition.kind === "food") {
      useSelectedApple();
    } else if (definition.kind === "build") {
      placeSelectedBuilding(definition);
    }
  }

  function releasePrimaryAction(source) {
    if (source === "mouse") game.mouseDown = false;
    if (source === "space") game.spaceDown = false;
    syncAttackIntent();
  }

  function toggleAutohit() {
    game.autohitActive = !game.autohitActive;
    updatePerformanceDisplay();
    syncAttackIntent();
  }
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.code === "Space") {
      event.preventDefault();
      if (!event.repeat && !game.openModal) pressPrimaryAction("space");
      return;
    }
  ...
  function updateAimFromPointer(clientX, clientY) {
    ...
    const pointerX = (clientX - rect.left) * canvas.width / Math.max(1, rect.width);
    const pointerY = (clientY - rect.top) * canvas.height / Math.max(1, rect.height);
    const screenX = self.x - game.camera.x;
    const screenY = self.y - game.camera.y;
    game.localAimAngle = Math.atan2(pointerY - screenY, pointerX - screenX);
  ...
  canvas.addEventListener("pointerdown", (event) => {
    ...
    pressPrimaryAction("mouse");
  });

  window.addEventListener("pointerup", (event) => {
    if (event.button === 0) releasePrimaryAction("mouse");
  });
  ...
  function pointIsInsideLake(pointX, pointY, lake) {
    if (
      pointX < lake.bounds.minX ||
      pointX > lake.bounds.maxX ||
      pointY < lake.bounds.minY ||
      pointY > lake.bounds.maxY
    ) return false;

    let inside = false;
    for (
      let currentIndex = 0, previousIndex = lake.points.length - 1;
      currentIndex < lake.points.length;
      previousIndex = currentIndex++
    ) {
      const current = lake.points[currentIndex];
      const previous = lake.points[previousIndex];
      const crosses = (current.y > pointY) !== (previous.y > pointY) &&
  ...
  function currentWorldRenderRadiusSquared() {
    const configuredRadius = game.config?.NETWORK?.interestRadius;
  ...
  function worldPointIsWithinRenderRange(x, y) {
    const self = game.players.get(game.selfId);
    if (!self) return false;
    const deltaX = x - self.x;
    const deltaY = y - self.y;
    return deltaX * deltaX + deltaY * deltaY < currentWorldRenderRadiusSquared();
  }

  function lakeIsWithinRenderRange(lake) {
    const self = game.players.get(game.selfId);
    if (!self) return false;
    if (pointIsInsideLake(self.x, self.y, lake)) return true;

    for (let index = 0; index < lake.points.length; index += 1) {
      const start = lake.points[index];
      const end = lake.points[(index + 1) % lake.points.length];
      if (
        pointToSegmentDistanceSquared(self.x, self.y, start, end) <
        currentWorldRenderRadiusSquared()
      ) return true;
    }
    return false;
  }
  ...
  function drawMapSurface() {
    ...
    for (const lake of game.lakes) {
      ...
      if (!style || !lakeIsWithinRenderRange(lake)) continue;
  ...
  function drawObstacle(obstacle, now) {
    ...
    if (!definition) return;
    if (!worldPointIsWithinRenderRange(obstacle.x, obstacle.y)) return;
  ...
  function drawBuilding(building, now) {
    if (!worldPointIsWithinRenderRange(building.x, building.y)) return;
  ...
  function drawPlayer(player, now) {
    if (player.id !== game.selfId && !worldPointIsWithinRenderRange(player.x, player.y)) return;
  ...
  function updateRenderPlayers(now, deltaMs) {
    const angleAmount = 1 - Math.exp(-deltaMs * REMOTE_ANGLE_LERP_RATE);
  ...
  function drawDebugShapes() {
    ...
    for (const obstacle of game.obstacles) {
      if (!worldPointIsWithinRenderRange(obstacle.x, obstacle.y)) continue;
    ...
    for (const building of game.buildings.values()) {
      const definition = getBuildDefinition(building.type);
      if (!definition || !worldPointIsWithinRenderRange(building.x, building.y)) continue;
    ...
    for (const player of game.players.values()) {
      if (player.id !== game.selfId && !worldPointIsWithinRenderRange(player.x, player.y)) continue;
      const x = player.x - game.camera.x;
      const y = player.y - game.camera.y;
  ...
  function updatePerformanceDisplay() {
    ...
    const autohit = game.autohitActive ? " autohitting" : "";
    dom.pingDisplay.textContent = `${fps} fps ${tps} tps ${ping} ping${autohit}`;
  }
